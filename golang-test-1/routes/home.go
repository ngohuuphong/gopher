...
func homePostHandler(w http.ResponseWriter, r *http.Request) {
	...
	if count > 0 {
		...
		for _, p := range products {
			html += "<tr>"
			html += fmt.Sprintf("<td> %d </td> <td> %s </td> <td> %s </td> <td> %.2f</td> <td> %d </td> <td> %.2f </td>", p.Id,
				p.Category.Description, p.Name, p.Price, p.Quantity, p.Amount)
			html += "</tr>"
		}
		html += "</table>"
	} else {
		html += fmt.Sprintf(`<p class='alert alert-info'> Item Not Found <code>"<strong> %s </strong> </code>"</p>`, search)
	}
